...
function releases_plugin_send_email( $p_project_id, $p_version, $p_files, $p_descriptions, $p_files_id )
{
    ...
    $t_subject = plugin_config_get( 'email_subject', PLUGINS_RELEASES_EMAIL_SUBJECT_DEFAULT );
    ...
    for( $i=0; $i<$p_files_count; $i++ ) {
        ...
        $t_template['files'][$i]['file_date'] = date( config_get( 'normal_date_format' ), plugins_releases_file_get_field( $p_files_id[$i], 'date_added'  ) );
    }
    ...
    $t_id_list = array();
    if ( plugin_config_get( 'notify_handler', PLUGINS_RELEASES_NOTIFY_HANDLER_DEFAULT ) == ON ) {
    ...
    // Get reporter
    if ( plugin_config_get( 'notify_reporter', PLUGINS_RELEASES_NOTIFY_REPORTER_DEFAULT ) == ON ) {
    ...
    // Add users
    $t_emails = explode( ',', plugin_config_get( 'notify_email', PLUGINS_RELEASES_NOTIFY_EMAIL_DEFAULT ) );
