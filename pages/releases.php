...
foreach ( $t_releases as $t_release ) 
{
    ...
    while ( $t_row = db_fetch_array( $t_result ) ) 
    {
        ...
        if ( $t_user_has_download_level ) {
            echo '<td width="25"><a class="btn btn-xs btn-primary btn-white btn-round releases_download" href="' . releases_plugin_page_url( 'download' ) . '?id=' . $t_row['id'] . form_security_param( 'plugin_Releases_download' ) . '&id=' . $t_row['id'] . '&enbl=' . ( $t_row['enabled'] ? '0' : '1' ) . '" title=" ' . plugin_lang_get( 'download' ) . ' ' . $t_row['title'] . '"> <i class="fa fa-download"></i> </a></td>';
        }
        
        echo '<td>&nbsp;' . $t_row['title'] . '</td>';
        ...
        if ( $t_user_has_upload_level ) {
            ...
            echo '<td width="55"><a class="btn btn-xs btn-primary btn-white btn-round releases_enable" href="' . plugin_page( 'enable' ) . form_security_param( 'plugin_Releases_enable' ) . '&id=' . $t_row['id'] . '&enbl=' . ( $t_row['enabled'] ? '0' : '1' ) . '" title=" ' . $t_enable_text . '">' . $t_enable_text . '</a></td>';
            echo '<td width="55"><a class="btn btn-xs btn-primary btn-white btn-round releases_delete" href="' . plugin_page( 'delete' ) . form_security_param( 'plugin_Releases_delete' ) . '&id=' . $t_row['id'] . '" title=" ' . lang_get( 'delete_link' ) . '">' . lang_get( 'delete_link' ) . '</a></td>';
        }
        echo '</tr></table></span>';
    }
...
?>
    ...
    <form action="<?php echo plugin_page( 'upload' ); ?>" method="post" enctype="multipart/form-data">
        <?php echo form_security_field( 'plugin_Releases_upload' ) ?>
